...
class StrategyBacktester:
    ...
    _PNL_COLS = ["pnl", "delta_pnl", "gamma_pnl", "theta_pnl", "vega_pnl", "residual_pnl", "leverage", "cashflow"]
    ...
    def compute_backtest(self, tcost_args: Optional[dict[str, Any]] = None) -> "StrategyBacktester":
        ...
        df_positions = self.apply_tcost(df_positions_raw, **(tcost_args or {})).sort_values(["option_id", "date"])
        ...
        df_nav = pd.DataFrame([[1.0]], columns=["NAV"], index=[start_idx])
        drifted_positions = []
        ...
        for current_date in sorted(df_positions["date"].unique()):
            ...
            latest_nav = df_nav.loc[current_date] if current_date in df_nav.index else df_nav.iloc[[-1]].iloc[0]
            df_nav.loc[current_date] = latest_nav + df_pnl.loc[current_date, "pnl"]
    ...
    def _preprocess_positions(df_positions: pd.DataFrame) -> pd.DataFrame:
        ...
        return ffill_options_data(df_positions_extended)
